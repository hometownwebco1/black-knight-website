      ...
      <section className="relative w-full h-[320px] md:h-[420px] lg:h-[480px]">
        ...
        <div className="relative z-10 h-full flex items-end">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
            ...
            <p className="mt-3 text-white/90 max-w-2xl">Real driveways, patios, walkways, and foundations built across Concord, NC and nearby.</p>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div key={project.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer" onClick={() => openModal(project)}>
              <div className="aspect-w-16 aspect-h-12 overflow-hidden">
                <img src={project.image} alt={project.title} className="w-full h-64 object-cover hover:scale-105 transition-transform duration-300" loading="lazy" />
              </div>
              <div className="p-6">
        ...
        <div className="text-center mt-16">
          ...
          <p className="text-lg text-gray-600 mb-8">Contact us today for a free estimate on your concrete project.</p>
          <Link to="/estimates"><Button size="lg" className="bg-primary hover:bg-primary/90">Get an Estimate</Button></Link>
        </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="relative max-w-4xl max-h-full">
            <button onClick={closeModal} className="absolute top-4 right-4 text-white hover:text-gray-300 z-10" aria-label="Close Image">
              <X size={32} />
            </button>
